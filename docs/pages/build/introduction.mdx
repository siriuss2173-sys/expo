...
export default function App() {
  const startSpectrometer = () => {
    Alert.alert("Spektrometre", "Spektral ölçüm başlatıldı!");
    // Burada telefon kamerası + spektrometre işleme kodları eklenecek
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Spektrometre Uygulaması</Text>
      <Button title="Ölçüm Başlat" onPress={startSpectrometer} />
    </View>
  );
}
